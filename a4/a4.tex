...
\begin{enumerate}[label=(\roman*)]
    \item We can do some algebra to solve for $s(t)$:
        \begin{alignat*}{2}
            &&Z(t) &= -y\ln{\left(\frac{g(t)}{M_b}\right)} \\
            \implies&&-\frac{Z(t)}{y} &= \ln{\left(\frac{g(t)}{g(t) + s(t)}\right)} \\
            \implies&&e^{-Z(t)/y} &= \frac{g(t)}{g(t) + s(t)} \\
            \implies&&g(t)+s(t) &= \frac{g(t)}{e^{-Z(t)/y}} = g(t)e^{Z(t)/y} \\
            \implies&&s(t) &= g(t)e^{Z(t)/y} - g(t) = g(t) (e^{Z(t)/y} - 1)
        \end{align*}
\end{enumerate}
